...
echo.
echo === 8) Mark one notification read (if exists) ===
curl -X PUT -H "Authorization: Bearer %WORK_JWT%" http://localhost:4000/api/notifications/read-all
echo.
echo === 9) Simulate wallet top-up for employer (simulate route) ===
curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer %EMP_JWT%" -d "{\"amount\":2500,\"currency\":\"INR\"}" http://localhost:4000/api/payments/simulate
echo.
...
echo.
echo === 12) Create & delete temp job (create/patch/delete) ===
...
REM Replace the following ID if create above returns different id; using last-temp id from logs 68c92cf407a14bc158b53e40
curl -X PATCH -H "Content-Type: application/json" -H "Authorization: Bearer %EMP_JWT%" -d "{\"title\":\"Temp Job Updated\"}" http://localhost:4000/api/jobs/68c92cf407a14bc158b53e40
curl -X DELETE -H "Authorization: Bearer %EMP_JWT%" http://localhost:4000/api/jobs/68c92cf407a14bc158b53e40
echo.
